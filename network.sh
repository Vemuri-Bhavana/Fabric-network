...
function checkPrereqs() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    infoln "CA_LOCAL_VERSION=$CA_LOCAL_VERCA_DOCKER_IMAGE_VERSION=$(${CONTAINER_CLI} run --rm hyperledger/fabric-ca:latest fabric-ca-client version | sed -ne 's/ Version: //p' | head -1)SION"
...
function createChannel() {
  ...
  scripts/createChannel.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE #calling create channel script
}
